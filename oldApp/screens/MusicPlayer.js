...
const MusicPlayer = ({navigation}) => {
    ...
    const [isPlayedSongIsLiked, setIsPlayedSongIsLiked] = useState(null);
    ...
    // console.log("timeNowBaretteDeShit==",timeNowBaretteDeShit)
    ...
    const rendListSongs = ({item}) => {
        ...
        return (
            <View style={{flexDirection:'row', flex:1, alignItems:'center', justifyContent:'space-evenly',paddingBottom:20, paddingHorizontal:20}}>
                <Image
                    source={{
                        uri: item.cover_url,
                    }}
                ...
                >
                    <View style={{width:180, paddingRight:20}}>
                        <MarqueeText speed={0.1} marqueeOnStart={true} delay={1000} loop={true} style={{
                            color: '#F4ECC4',
    ...
    useEffect(() => {
        logInput();
    }, [shouldShow3, titleNow]);

    const newCache = async () => {
        const favoriteList = await AsyncStorage.getItem('@songliked');
        const tempList = JSON.parse(favoriteList) ? JSON.parse(favoriteList) : []
        ...
        if (tempList?.length <= 0) {
            ...
            setShouldShow3(true);
            await AsyncStorage.setItem('@songliked', JSON.stringify(tempList))
        } else if (favoriteList && favoriteList?.length > 0) {
            const found = tempList.some(el => el.coverliked === coverAlbum);
            ...
            tempList?.forEach((item, index) => {
                if (item?.coverliked === coverAlbum) {
                    tempList.splice(index, 1)
                    setShouldShow3(false)
                } else if (item?.coverliked != coverAlbum) {
                    tempList.push({
                        titleliked: titleNow,
                        coverliked: coverAlbum,
                        artistliked: artistNow
        ...
        setIsPlayedSongIsLiked(coverAlbum)
        setLikedSongs(tempList.reverse())
    ...
    const deleteFirstItem = async () => {
        let tempo_likedSings = likedSongs;
        tempo_likedSings.shift();
        setLikedSongs(tempo_likedSings);
        await AsyncStorage.removeItem('@songliked')
        await AsyncStorage.setItem('@songliked', JSON.stringify(tempo_likedSings));
        setShouldShow3(false);
    }

    const deleteItemByIndex = async (index) => {
        let tempo_likedSings = likedSongs;
        tempo_likedSings.splice(index, 1);
        setLikedSongs(tempo_likedSings);
        await AsyncStorage.removeItem('@songliked')
        await AsyncStorage.setItem('@songliked', JSON.stringify(tempo_likedSings));
        if (index === 0) {
    ...
    const renderSong = ({item}) => {
        return (
            <View style={{flexDirection:'row', flex:1, alignItems:'center', justifyContent:'space-evenly',paddingBottom:20, paddingHorizontal:20, backgroundColor: '#3C1128'}}>
                <Image
                    source={{
                        uri: item.coverliked,
                    }}
                ...
                <View style={{flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center'}}>
                    <View style={{width:180, paddingRight:20}}>
                        ...
                        }}>{item.titleliked}</MarqueeText>
                        <MarqueeText speed={0.1} marqueeOnStart={true} delay={1000} loop={true} style={{
                            color: '#CDC7A7',
                            fontFamily: 'Futura',
                            fontSize: 11,
                            fontWeight: '500',
                        }}>{item.artistliked}</MarqueeText>    
                    </View>
                    <Pressable onPress={() => {
                        navigation.navigate('DetailSongLike', {
    ...
    const renderHiddenItem = (data, rowMap) => (
        <View style={style.rowBack}>
            <TouchableOpacity
                ...
                onPress={() => deleteItemByIndex(data.index)}>
        ...
        <SafeAreaView style={style.container}>
            ...
            <View style={style.mainContainer}>
                ...
                <Image
                    ...
                    style={{aspectRatio: 1, width: '90%', borderRadius: 10}}
                />
                {/* Titre & Artiste */}
                <View style={style.playerInfos}>
                    <View style={{}}>
                        <MarqueeText speed={0.1} marqueeOnStart={true} delay={1000}
                    ...
                    <View style={{backroundColor: 'white'}}>
                        <View style={style.progressBar}>
                            <ProgressBar progress={(timeNowBaretteDeShit / duration) * 100} width={143} height={3}
                                         backgroundColor="#FA7C00" trackColor="#BEB7a0"/>
                        </View>
                ...
                <View style={style.precedement}>
                    <View style={style.headBottom}>
                        ...
                        <View style={style.toggleButtonPosition}>
                            {shouldShow ? (
                                    <Image source={require("../assets/img/inactivelist.png")} style={{
                                        width: normalize(14),
                                        height: normalize(14),
                                        zIndex: 10,
                                        position: 'absolute',
                                ...
                                <Image source={require("../assets/img/inactivelike.png")} style={{
                                    width: normalize(14),
                                    height: normalize(14),
                                    zIndex: 10,
                                    position: 'absolute',
                            ...
                            <ToggleButton
                                ...
                                thumbActiveComponent={
                                    <Image source={require("../assets/img/likeIcon.png")} style={style.overIcon}
                                           fill={'#FFCA00'}/>
                                }
                                thumbInActiveComponent={
                                    <Image source={require("../assets/img/precedentIcon.png")} style={style.overIcon2}
                                           fill={'#3E64D4'}/>
                                }
                                trackBar={{
                    ...
                    <View style={style.programBanner}>
                        <Pressable onPress={() => navigation.navigate('Calendar')}>
                            <Text style={{
                                ...
                                fontSize: 20,
                                color: '#3C1128'
                            }}>+ {currentEventStart}h-{currentEventEnd}h - {currentEvent}</Text>
                        </Pressable>
                    </View>
                </View>
            </View>
...
const style = StyleSheet.create({
    ...
    buttonOnComponent: {
        ...
        justifyContent: 'center',
        zIndex: -1,
        flex: 1, // This will make the component expand to fill available space
        alignSelf: 'stretch', // This ensures the component takes up full width
    },
    rowFront: {
        alignItems: 'center',
        backgroundColor: '#3C1128',
